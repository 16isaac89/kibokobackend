...
Route::group(['prefix' => 'admin', 'as' => 'admin.', 'namespace' => 'Admin', 'middleware' => ['auth', '2fa']], function () {
    ...
    Route::post('/customers/update-latlong', 'CustomerController@importLatLong')->name('customers.importLatLong');
...
//Exec reports
    ...
    Route::get('/main/dealer/dispatch', 'DispatchController@dealerDispatch')->name('dealer.dispatch.maindealer');
    Route::get('/main/dealer/dispatch/{dispatch}', 'DispatchController@dealerDispatched')->name('partner.dispatched.items');

    Route::post('/main/dealer/updatedispatch', 'DispatchController@updatedispatched')->name('dealer.update.dispatched');
    Route::post('/main/dealer/searchdispatch', 'DispatchController@dealergetDispatch')->name('partner.get.dealerdispatches');
...
Route::get('excel/check', function () {
    $filePath = storage_path('app/public/faida.xlsx'); // Path to your Excel file
    ...
    $dealerColumn     = 'B'; // Adjust based on your actual dealer column
    $dealerCodeColumn = 'G'; // Adjust based on your actual dealer_code column

                                                  // Loop through each row and update dealer_code
    for ($row = 2; $row <= $highestRow; $row++) { // Assuming row 1 contains headers
        $dealer = $sheet->getCell($dealerColumn . $row)->getValue();
        if ($dealer === "NKUMBA") {
            $sheet->setCellValue($dealerCodeColumn . $row, 'FAIDA022');
        }
    }

    // Save the updated file (overwrite the existing customers.xlsx)
    $writer = IOFactory::createWriter($spreadsheet, 'Xlsx');
    $writer->save($filePath);

    return "customers.xlsx has been updated successfully!";

    // return response()->download($updatedFilePath);
