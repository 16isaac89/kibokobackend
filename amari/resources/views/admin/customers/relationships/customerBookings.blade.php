...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-customerBookings">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <th>
                            {{ trans('cruds.booking.fields.triptype') }}
                        </th>
                        <th>
                        ...
                        <th>
                            {{ trans('cruds.booking.fields.from') }}
                        </th>
                        <th>
                            {{ trans('cruds.booking.fields.to') }}
                        </th>
                        <th>
                            {{ trans('cruds.booking.fields.paymentmethod') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                ...
                <tbody>
                    @foreach($bookings as $key => $booking)
                        <tr data-entry-id="{{ $booking->id }}">
                            ...
                            <td>
                                {{ App\Models\Booking::TRIPTYPE_SELECT[$booking->triptype] ?? '' }}
                            </td>
                            <td>
                                @foreach($booking->services as $key => $item)
                                    <span class="badge badge-info">{{ $item->name }}</span>
                                @endforeach
                            </td>
                            <td>
                                {{ App\Models\Booking::TYPE_RADIO[$booking->type] ?? '' }}
                            </td>
                            <td>
                                {{ $booking->from ?? '' }}
                            </td>
...
@endcan
  ...
  let table = $('.datatable-customerBookings:not(.ajaxTable)').DataTable({ buttons: dtButtons })
