...
@include('dealer.reports.modals.repsales')
    <div class="card pd-20 pd-sm-40">
      <div class="card-body">
        ...
        <input type="hidden" value="{{$year}}" id="routeyear">
          <form class="form-inline" method="post" action="{{route('search.reporders')}}">
            @csrf
            <input class="form-control mr-sm-2 month" type="search" name="month" placeholder="Search" aria-label="Search">
            <input class="form-control mr-sm-2 year" type="search" name="year" placeholder="Search" aria-label="Search">
            <button class="btn btn-outline-success my-2 my-sm-0" type="submit">Search</button>
          </form>
          </div>
        </div>
        <h5 class="card-title">Rep Wise Report</h5>
        ...
        <table id="routes"  class="table display responsive nowrap">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                <button class="btn btn-primary" data-toggle="modal" data-target="#repsales" data-id="{{$sale->id}}" data-name="{{$sale->username}}">
                  View
                </button>
               </td>
            </tr>
...
<script>
  $('#repsales').on('show.bs.modal', function (event) {
    var button = $(event.relatedTarget) // Button that triggered the modal
    var id = button.data('id') // Extract info from data-* attributes
