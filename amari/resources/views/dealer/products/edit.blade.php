...
<div class="card">
    ...
    <div class="card-body">
        ...
        <form method="post" action="{{route('dealer.product.edit')}}">
        @csrf
...
@if(\Auth::guard('dealer')->user()->dealer->efris === 1)
  <div class="col-6" style="margin:10px;">
    ...
    <input name="categorycode" type="text" {{\Auth::guard('dealer')->user()->dealer->efris === 1 ? 'required':'' }}  value="{{$product->efriscategorycode}}" class="form-control" placeholder="Product Efris Category Code ">
  </div>
  </div>
@endif
